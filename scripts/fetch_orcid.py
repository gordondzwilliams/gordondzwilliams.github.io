#!/usr/bin/env python3
# scripts/fetch_orcid.py (aggressive extraction + recursive name scan)
...
# Heuristic: string looks like a person name
def looks_like_name(s):
    if not s or not isinstance(s, str):
        return False
    s = s.strip()
    # Reject if very long or contains url/doi/email
    if len(s) > 80:
        return False
    if "http" in s or "doi.org" in s or "@" in s:
        return False
    # Accept if contains at least one space and letters (e.g. "Gordon D. Williams")
    if " " in s and re.search(r"[A-Za-z]", s):
        # avoid strings that are just affiliations like "Department of..."
        if re.search(r"\b(dept|department|univ|university|institute|school|college|laboratory|lab)\b", s.lower()):
            return False
        # avoid short two-letter codes like "US EPA" (allow longer)
        if len(s.split()) > 5:
            return False
        return True
    # Or names in "Lastname, Firstname" form (contains comma)
    if "," in s and re.search(r"[A-Za-z]", s) and len(s) < 80:
        return True
    return False

def deep_collect_strings(obj, max_found=50):
    """Recursively collect string values from obj and return candidates that look like names."""
    found = []
    stack = [obj]
...
def find_authors_targets(summary, item):
    ...
    if summary and isinstance(summary, dict) and summary.get("contributors"):
        ...
        for e in entries:
            ...
            if n:
                authors.append(n)
    # item-level contributors / work object
    if not authors:
...
def parse_group_item(item, idx):
    # item is the `group` object in activities-summary.works.group
    ...
    title_raw = title_raw or summary.get("title')") if False else None  # placeholder - ignore weird tokens
    ...
    if not authors:
        ...
        # Keep plausible candidates as authors (top 6)
        if candidates:
            used_deep = True
...
def main():
    ...
    written = []
    # write small debug file with first few groups
    try:
    ...
    for i, item in enumerate(works_group):
        parsed = parse_group_item(item, i)
